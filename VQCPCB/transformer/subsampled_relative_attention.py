...
class SubsampledRelativeAttention(nn.Module):
    ...
    def forward(self, q):
        ...
        masks_down = torch.triu(torch.ones_like(rel_attn_1[0, :self.seq_len_src, :self.seq_len_src]).byte(),
            ...
            1).flip(2).type(torch.bool)
        if self.subsampling_ratio != 1:
            masks_down = torch.repeat_interleave(masks_down, self.subsampling_ratio, dim=1)

        masks_up = torch.triu(torch.ones_like(rel_attn_1[0, :self.seq_len_src, :self.seq_len_src]).byte(),
                              diagonal=1).unsqueeze(0).repeat(sz_b_times_n_head, 1, 1).type(
            torch.bool)
        if self.subsampling_ratio != 1:
